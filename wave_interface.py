...
class Comb(IWave):
    ...
    def load_models(self, tac_model="./checkpoints/tacotron2_statedict.pt",
        ...
        from mel_interface import load_mel_model
        mel_model = load_mel_model(tac_model)

        waveglow = torch.load(wav_model)['model']
        waveglow = waveglow.remove_weightnorm(waveglow)
        waveglow.cuda().eval()
        # TODO: Make fp16 part of args
        if is_fp16:
            from apex import amp
            waveglow, _ = amp.initialize(waveglow, [], opt_level="O3")

        # TODO: Make denoiser part of args
        denoiser = None
    ...
    def inference_audio(self, text, outdir="./output", batch=1, implementation="auto", is_fp16=True, sigma=0.6):
        text = self.preprocess(text)
        start = time.time()
        from mel_interface import inference_mel
